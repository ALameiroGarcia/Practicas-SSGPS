...
def conversorUTM(latitud, longitud, datum, tiempo):
    ...
    return UTM_Easting, UTM_Norting, tiempo, latitud, longitud
...
def updateMap(latitud, longitud, map_instance, map_type):
    # Configura tu clave de API de Bing Maps
    api_key = 'AgP8u7cVdPjPJ11gIP96H01UDBl7KnZ04-DgPFDBsUfyk6m6OFrZg6lP2ruiCCjG'
    zoom = 19
    # Construye la URL de la solicitud
    url = f"https://dev.virtualearth.net/REST/v1/Imagery/Map/{map_type}/{latitud},{longitud}/{zoom}?mapSize=500,500&key={api_key}"

    # Realiza la solicitud a la API de Bing Maps
    respuesta = requests.get(url)

    # Verifica si la solicitud fue exitosa
    if respuesta.status_code == 200:
        # Guarda la imagen en un archivo
        with open('imagenes/imagen_aerea.jpg', 'wb') as archivo:
            archivo.write(respuesta.content)   
        # Actualiza la imagen en la aplicación
        map_instance.update_image("imagenes/imagen_aerea.jpg")
    else:
